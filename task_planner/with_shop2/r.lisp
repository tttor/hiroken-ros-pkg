...
(defdomain tidy-up 
  (
    ...
    (:method (pick-and-place ?l1 ?l2)
      ( (in ?o ?l1) )
      (:ordered (!transit ?r ?l0 ?l1) (!grasp ?r ?o ?l1) (!transfer ?r ?o ?l1 ?l2) (!ungrasp ?r ?o ?l2) ); STUNNING what binds ?l0 ? A: the precondition of !transit
    )
    (:method (tidy ?l1 ?l2 ?r ?home)
      ( (in ?o ?l1) )
      (:ordered (pick-and-place ?l1 ?l2) (tidy ?l1 ?l2 ?r ?home) )
      ( )
      ( (!transit ?r ?l2 ?home) ); ( (!transit ?r ?l2 ?l0) ); BUT NONE binds ?l0 here, why??? Workaround: pass the ?home, plus ?r for the sake of intuitiveness
    )
  )
)
