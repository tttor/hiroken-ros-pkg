...
#define DLARM  (LARM)
  ...
  public:
     ...
     * @return     デジタル入力のステータス情報。
     */ 
    unsigned long long getDin();

	/*!
     * @brief      デジタル出力を設定。
	 * @param[in]  port ポート番号
	 * @param[in]  blinkingCycle
	 *                - 0x00000000 : OFF < @c #DO_OFF >
     *                - 0xFFFFFFFF : ON  < @c #DO_ON  >
     *                - 上記以外 : 点滅 （ON / OFF 繰り返し） \n
     *                  点滅周期を 10msec 単位で指定します （デューティ比 50\%）。\n
     *                （例） @a blinkingCycle = 100 の場合，点滅周期 1sec \n
     * @return     true:成功、false:失敗（無効ポートを指定した場合）
     */
    bool setDout(unsigned short port, unsigned long blinkingCycle);
     ...
     */
	unsigned long getDout(unsigned short port);

	 /*!
     * @brief      デジタル出力の値を取得します。
     * @return     デジタル出力のステータス情報。
     */
